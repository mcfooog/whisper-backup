...
本模型是 [**FunASR**](https://github.com/modelscope/FunASR) 语音生态的组件模型之一,可与 ASR 模型自由组合成完整流程(VAD 切分 · 标点恢复 · 说话人分离 · 情感识别)。FunASR 一行代码即可调用本模型。
...
```shell
funasr vad_example.wav --model paraformer    # 本模型作为 paraformer 流程中的组件(VAD/标点等)
```
