# python zimu_en.py "D:\yt\video.mp4"
...
def transcribe_to_srt(
    ...
    if out_dir:
        ...
        base_name = os.path.splitext(os.path.basename(input_path))[0] + "_en.srt"
        out_file_candidate = os.path.join(out_dir, base_name)
    else:
        out_file_candidate = os.path.splitext(input_path)[0] + "_en.srt"
    ...
    print(f"✅ 英文字幕生成完成，已保存: {os.path.abspath(out_file)}", flush=True)
    return os.path.abspath(out_file)


def main():
    parser = argparse.ArgumentParser(description="基于 faster-whisper 的英文字幕生成")
